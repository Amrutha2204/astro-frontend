...
export default function MonthlyHoroscopePage() {
  ...
  const fetchHoroscope = async () => {
    try {
      ...
      setLoading(true);
      const data = await horoscopeApi.getMonthlyHoroscope(token);
      console.log("📆 Monthly Horoscope API Response:", JSON.stringify(data, null, 2));
      setHoroscope(data);
      setError(null);
    } catch (err) {
      const error = err as { message?: string };
      const errorMessage = error.message || "Failed to load Monthly Horoscope";
      setError(errorMessage);
      console.error("Error fetching Monthly Horoscope:", err);
