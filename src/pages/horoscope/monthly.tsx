...
export default function MonthlyHoroscopePage() {
  ...
  const fetchHoroscope = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await horoscopeApi.getMonthlyHoroscope(t);
      setHoroscope(data);
      setError(null);
    } catch (err) {
      const e = err as { message?: string };
      const msg = e.message || "Failed to load Monthly Horoscope";
      setError(msg);
                    ...
                    <div className="mt-8">
                      ...
                      <div className="mt-5 grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
                        {horoscope.predictions.map(
                          (prediction: MonthlyPrediction, index: number) => {
